...
class NoiseSuppressorWorklet extends AudioWorkletProcessor {
   ...
   */
  private _denoiseProcessor: RnnoiseProcessor | undefined;
   ...
   */
  private _denoiseSampleSize: number | undefined;

  /**
   * Circular buffer data used for efficient memory operations.
   */
  private _circularBufferLength: number | undefined;

  private _circularBuffer: Float32Array | undefined;

  /**
   * The circular buffer uses a couple of indexes to track data segments.
   * Input data from the stream is
  ...
  constructor() {
     ...
     * initialization process does not wait for the resolution
     * of promises in the AudioWorkletGlobalScope.
     */
    (async () => {
      const wasmModule = await createRNNWasmModuleSync();
      this._denoiseProcessor = new RnnoiseProcessor(wasmModule as any);
       ...
       * the residue could potentially be split between the end of buffer and beginning and would
  ...
  /**
   * Worklet interface process method.
   * The inputs parameter contains PCM audio that is then sent to rnnoise.
   * Rnnoise only accepts PCM samples of 480 bytes whereas `process` handles 128 sized samples,
   * we take this into
   * account using a circular buffer.
   *
   * @param {Float32Array[]} inputs - Array of inputs connected to the node,
   * each of them with their associated array of channels.
   * Each channel is an array of 128 pcm samples.
   * @param {Float32Array[]} outputs - Array of outputs similar to the inputs parameter structure,
   * expected to be filled during the execution of `process`.
   * By default each channel is zero filled.
   * @returns {boolean}-Boolean value that returns whether or not the processor should remain active
   * Returning false will terminate it.
   */
  process(inputs: Float32Array[][], outputs: Float32Array[][]) {
    // We expect the incoming track to be mono,
    // if a stereo track is passed only on of its channels will get
    // denoised and sent pack.
    // TODO Technically we can denoise both channel however this might require a new rnnoise context
    ...
    if (
      ...
      && this._denoiseSampleSize
      && this._circularBufferLength
      && this._denoiseProcessor
    ) {
      ...
      // if the start index of denoised samples is smaller
      ...
      // e.g. if the buffer size is 1024 samples but we only denoised 960
      // (this happens on the first iteration) nothing happens,
      // then on the next iteration 1920 samples will be denoised so we send 1024 which leaves
      ...
      // When the end of the circular buffer has been reached, start from the beginning.
      // By the time the index starts over,
      // the data from the begging is stale (has already been processed) and can be safely
      // overwritten.
      if (this._denoisedBufferIndx === this._circularBufferLength) {
        this._denoisedBufferIndx = 0;
      }

      // circular buffer's length is the lcm of both input size and the processor's sample size,
      // by the time we reach the end with the input index
      // the denoise length index will be there as well.
...
registerProcessor("NoiseSuppressorWorklet-ts", NoiseSuppressorWorklet);
